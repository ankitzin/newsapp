...
export class News extends Component {
    ...
    async componentDidMount(){
        ...
        let url = `https://newsapi.org/v2/top-headlines?country=de&category=sports&apiKey=ada6ce19b55a4330adf52d7b6bae2db4&page=1&pageSize=${this.props.pageSize}`
    ...
    handlePrevious= async() =>{
        ...
        let url = `https://newsapi.org/v2/top-headlines?country=de&category=sports&apiKey=ada6ce19b55a4330adf52d7b6bae2db4&page=${this.state.pageNum}&pageSize=${this.props.pageSize}`
        this.setState({loading:true})
        let data = await fetch(url)
        let parsedData = await data.json()
        this.setState({
    ...
    handleNext= async()=>{
        ...
        if(this.state.pageNum + 1>Math.ceil(this.state.totalResults/this.props.pageSize)){
            ...
            let url = `https://newsapi.org/v2/top-headlines?country=de&category=sports&apiKey=ada6ce19b55a4330adf52d7b6bae2db4&page=${this.state.pageNum +1}&pageSize=${this.props.pageSize}`
            this.setState({loading:true})
            let data = await fetch(url)
            let parsedData = await data.json()
            this.setState({
      ...
      <>
        <div className="container my-3">
            ...
            <div className="container d-flex justify-content-between">
                <button disabled={this.state.pageNum<=1} type="button" className="btn btn-info mx-2" onClick={this.handlePrevious}>&larr; Previous</button>
                <button disabled={this.state.pageNum + 1>Math.ceil(this.state.totalResults/this.props.pageSize)} type="button" className="btn btn-info mx-2" onClick={this.handleNext}>Next &rarr;</button>
            </div>
        </div>
      </>
    )
